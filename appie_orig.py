...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///annie.db'
...
def make_results_table():
	if request.method == 'POST':
		if request.form.get('Generate results') == 'Generate results':        	
			...
			conn.commit()
			cursor.execute("INSERT INTO final VALUES ('Ulster Scots: ', '0'), ('Derry English: ','0'), ('Dublin English: ', '0'), ('Kerry English: ', '0') ")
			conn.commit()

			cursor.execute("UPDATE final SET percentage = ROUND(total2.ulsterscots,0) FROM total2 AS total2 WHERE final.variety = 'Ulster Scots: '")
			conn.commit()
			cursor.execute("UPDATE final SET percentage = ROUND(total2.derry,0) FROM total2 AS total2 WHERE final.variety = 'Derry English: '")
			conn.commit()
			cursor.execute("UPDATE final SET percentage = ROUND(total2.dublin,0) FROM total2 AS total2 WHERE final.variety = 'Dublin English: '")
			conn.commit()
			cursor.execute("UPDATE final SET percentage = ROUND(total2.ruralsw,0) FROM total2 AS total2 WHERE final.variety = 'Kerry English: '")
			conn.commit()
			cursor.execute("SELECT * FROM final ORDER BY percentage DESC")
			results_data = cursor.fetchall()
		conn.commit()
